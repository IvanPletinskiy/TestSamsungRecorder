...
/* renamed from: com.sec.android.app.voicenote.ui.view.SimpleFloatingView */
public class SimpleFloatingView extends LinearLayout {
    private static final String TAG = "SimpleFloatingView";
    ...
    public SimpleFloatingView(Context context, AttributeSet attributeSet, int i) {
        super(context, attributeSet, i);
        init();
    }

    public SimpleFloatingView(Context context, AttributeSet attributeSet) {
        super(context, attributeSet);
        init();
    }

    public SimpleFloatingView(Context context) {
        super(context);
        init();
    }

    public void init() {
        this.mEmptyWidth = (((float) WaveProvider.SIMPLE_WAVE_AREA_WIDTH) * 1.0f) / 2.0f;
    ...
    public void setX(float f, float f2) {
        if (getVisibility() != 8) {
            if (!this.mIsFixed) {
                float f3 = 0.0f;
                SimpleEngine engine = SimpleEngineManager.getInstance().getEngine(this.mSession);
                if (engine != null) {
                    f3 = (((float) engine.getDuration()) / WaveProvider.MS_PER_PX) + (this.mEmptyWidth * 2.0f);
                }
